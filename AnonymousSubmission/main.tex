...
using Python libraries they are already familiar with.
...
By adding the additional step for clients to request for an ML model to train,
we make it trivial to deploy new models as long as the new models take the same
kind of inputs and outputs.

\subsubsection{Optional Telemetry}
During each stage of training, we allow options to record telemetry information
for analytics.
