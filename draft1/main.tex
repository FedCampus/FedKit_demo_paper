...
FedKit facilitates FL among clients (mobile apps) \textit{across} Android and iOS platforms,
coordinated by a single Backend server.
Each client trains a local model with private data,
and the Backend performs cross-aggregation of these local models to update the global model.
...
FedKit leverages TFLite and Core ML to locally train the converted models on different devices,
utilizing GPU and NPU acceleration.
Our \texttt{trainers} expose unified APIs for
\lstinline{retrieving} and \lstinline{setting} parameters,
\lstinline{fitting}, and \lstinline{evaluation} across different platforms.
