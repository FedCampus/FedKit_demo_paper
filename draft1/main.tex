...
are prime candidates for FL participation.
For feasible FL experiments on smartphones,
a comprehensive system must support cross-platform on-device training and
provide flexibility to make changes in production.

However, existing FL systems often fall short in
multiple requirements for feasible FL experiments on smartphones.
...
Some, like FedML \cite{he2020fedml}, Project Florida \cite{madrigal2023project}
and FedScale \cite{lai2022fedscale},
merely support Android.
Proprietary services such as FedML and Florida cannot be self-hosted,
limiting customization possibilities.
Flower \cite{beutel2020flower,mathur2021ondevice} provides only
an iOS SDK,
...
these systems are also less conducive to research iteration in
production settings,
primarily supporting one-shot experiments.

\subsubsection{Contributions}
We developed FedKit,
an FL system to enable cross-platform FL research for
iOS and Android:
\begin{enumerate}[label=\bullet]
    \item FedKit provide a set of modularized libraries to train
        and aggregate Python-based ML models on Android and iOS.
        These libraries can serve as a foundation or reference for
        future FL systems.
    \item We exemplify a novel FL workflow to control ML models and
        FL algorithms from the backend in production settings,
        inspiring other open-source solutions to compete in this domain with
...
We introduce FedCampus, a campus-based FL experiment using exercise data from
participants' smartwatches.
We deployed FedKit in the FedCampus research app,
which trains a sleep-efficiency prediction model
similar to \cite{khoa2022fedmcrnn}
using data from Huawei HealthKit on Android and Apple Health on iOS.
Our experiment engaged three test users who contributed data,
resulting in a noticeable reduction in training loss,
thereby demonstrating the effective functionality of FedKit.
