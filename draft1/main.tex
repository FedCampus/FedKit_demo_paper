...
\begin{abstract}
    In this paper, we present FedKit, a Federated Learning (FL) system tailored for
    ...
    Furthermore, our FL workflow supports flexible ML operations (MLOps) in production,
    facilitating continuous model delivery and training.
    To illustrate the practical utility, we have deployed FedKit in a research application as a use case,  
...
provide SDKs for both Android and iOS platforms.
However, Flower SDKs lack the capability for cross-platform aggregation, while PySyft lacks support for hardware acceleration for training.

In addition to seamless cross-platform support,
another crucial challenge is continuous deployment, particularly for ML operations (MLOps) in production settings. 
While FedML and Florida support MLOps,
their proprietary services limit full customization.
In contrast, open-source solutions like Flower and PySyft enable
full customization
but are more suitable for one-shot experiments rather than continuous production.
