...
\begin{abstract}
    ...
    enabling conversion of Python implemented models,
    native performance training with unified APIs,
    and cross-Android-iOS model aggregation.
    FedKit supports flexible ML operations (MLOps) in production,
...
For TFLite, we \textit{standardized} the model format by developing a TensorFlow converter.
The standardized model format is equipped with
...
locally train the converted models on Android and iOS devices. 
The trainers leverage corresponding frameworks 
utilizing GPU and NPU acceleration for on-device training. 
Moreover, both trainers expose unified APIs for
\textit{retrieving and setting model parameters,
model fitting and evaluation}.
On Android, these unified APIs invoke the TFLite interpreter to call
