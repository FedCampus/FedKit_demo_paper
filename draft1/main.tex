...
\title{FedKit---Enabling Cross-Platform (Android/iOS) Federated Learning}
\author{}
\affiliations{}
...
\begin{abstract}
    We present FedKit, a federated learning (FL) system to facilitate real-world
    FL experiments across Android and iOS.
    FedKit allows for FL for the same models across Android and iOS,
    ...
    It is open-source at \url{https://github.com/FedCampus/FedKit}.
\end{abstract}

\section{Introduction}

Federated learning (FL) is a privacy-preserving machine learning (ML) technique.
It allows edge devices to collaboratively train a shared ML model while
keeping training data local.
FL on smartphones is especially promising because of the huge amount of
user data.

Unfortunately,
existing FL systems often lack functionalities to support general FL research
across smartphone platforms.
Most do not run on smartphones
\cite[e.g.,][]{bonawitz2019towards,liu2021fate,ma2019paddlepaddle,openfl_citation}.
A comparison among some other systems is presented in Table \ref{tbl:fn-frameworks}.
FedML \cite{he2020fedml} and Project Florida \cite{madrigal2023project}
only support Android,
and their proprietary backends cannot be self-hosted and fully controlled.
Flower \cite{beutel2020flower,mathur2021ondevice} only has an official iOS SDK.
PySyft \cite{ryffel2018generic,Ziller2021,hall2021syft} uses a custom
on-device training implementation that is significantly slower than native ones.
% TODO: Cite more.
