...
Smartphones are an ideal participant for FL due to the substantial volume of
sensitive data they handle.
However, FL on smartphones is challenging due to the complexity in mobile
development and on-device training.

Existing FL systems often fall short in
...
FedML and Florida are proprietary services and cannot be self-hosted,
so direct customizations are impossible.
...
In response to these challenges, we developed FedKit,
an open-source system designed to facilitate cross-platform FL research and
enables flexible in-production customizations.
FedKit offers convenience for researchers to conduct and iterate on
FL experiments across Android and iOS,
while maintaining modularity,
allowing further customizations by reimplementing specific modules.
As a result, FedKit can serve as a foundational resource or reference for
the broader FL research community.
