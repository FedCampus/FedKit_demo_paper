...
FL enables edge devices to
collaboratively train a shared machine learning (ML) model while
retaining private local data.
Smartphones, rich in sensitive data,
are promising candidates for FL.
Nevertheless, deployed mobile FL systems for real-world experiments are rare.
Our end goal is to build such a platform for
continuous FL research using smartphone data on DKU campus.

Existing on-smartphone FL systems could not satisfy our requirements to
