...
we aim to build a practical mobile FL platform for
harnessing real users' health data on university campuses,
which we named the \FedCampus{} project.
...
two clients running an example Flutter client app,
and a laptop running a \FedKit{} Backend.
First, our \textit{FL model pipeline} is demonstrated.
...
we modify the model and deploy the new version.
As outlined in Table~\ref{tbl:demo-stats},
the real-world statistics shows that
