...
\begin{abstract}
    ...
    exercise data analysis on Duke Kunshan University (DKU) campus,
...
We run an MNIST model written in Python through our model pipeline,
and deploy it on a laptop running a \FedKit{} server instance.
We then run FL across an Android and an iOS devices,
as outlined in Table~\ref{tbl:demo-stats}.
Finally, we modify the model and deploy the new version to showcase MLOps.
...
we self-hosted the Backend in DKU,
